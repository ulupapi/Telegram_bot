...
- интерфейс через Reply Keyboard (кнопки внизу чата), включая:
  - отправку общей сводки по кнопке;
  - справку по работе кнопок;
...
- разделение интерфейсов:
  - пользовательский: только кнопки `Получить общую сводку`, `Помощь`;
  - режим программиста: включается из `Помощь`, дает кнопки `Очистить текущий контекст`, `Очистить всю БД`, `Текущий контекст`, `Параметры расписания`.
- пользователь не создает задачи вручную: бот формирует их из собранного контекста через LLM при запросе сводки.
