...
- интерфейс через Reply Keyboard (кнопки внизу чата), включая:
  - обновление сводки;
  - переход в режим редактирования задач;
...
- автоочистка предыдущего пакета сообщений сводки перед новой отправкой;
- разделение интерфейсов:
  - пользовательский: `menu`, `summary`, `edit_task`;
  - dev-команды (в личке с ботом): `dev_help`, `dev_where`, `dev_clear`, `dev_clear_all`, `dev_schedule`.
- ручное добавление задачи по reply-команде: `/add_task` (не показывается в пользовательском меню команд).
